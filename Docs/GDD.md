...
- The game starts in one main pantry room, then expands into a small number of connected specialty rooms later.
...
- Compact mode summarizes the whole pantry and calls out the room that needs attention instead of forcing room-by-room micromanagement.
...
- first justified second-room unlock
- first prestige

Room progression should follow this structure:

- Prototype and demo: one main pantry room
- Early Access core: two to three connected rooms
- 1.0 normal end-state: three strongly themed rooms, with a fourth only if it clearly adds value
...
- one complete chain readable in one room view
...
- connected pantry rooms with clear room roles
...
- understand which room is currently helping or blocking featured demand
...
- room expansion feels earned and readable instead of like menu sprawl
